...
def get_components():
    ...
    cfgPaths=['cfg.txt', 'ref_cfgs.txt', 'work_cfgs.txt']
    cfgPath=""
    zPath=""
    output=""
    componentsLst=[]
    ...
    #MMS: modified list of *cfg*.txt - work_cfgs.txt was missing here....
    #if release3 == True:
        #cfgPath = open(getNemoConfig() + "/cfg.txt", "r")
    #elif release4 == True:
        #cfgPath = open(getNemoConfig() + "/ref_cfgs.txt", "r")
        
    for zPath in cfgPaths:
        if os.path.isfile(currentDirectory + "/" + zPath)
            cfgPath = open(currentDirectory + "/" + zPath, "r")
            try:
                for i in cfgPath:
                    if exp in i:
