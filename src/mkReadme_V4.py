...
def getNemoConfig():
    '''This function will look for the directory NEMOGCM/CONFIG anywhere in the system'''

    #returns (/root)
    if scriptLoc != None:
        rootPath = os.path.expanduser('~') #to be used when script is saved in a configuration
    else:
        rootPath = pathlib.Path().absolute() #to be used when script is saved in the root
    fname = []
    configPath=""
...
def check_EXPref():
    '''This function will check whether the current location contains and EXP00 folder'''
    path = os.getcwd()
    expRef = False
...
def checkScriptLocation():
    ...
    for x in getAllConfigFolders():
        if x == configName:
            #print("This script is saved in a configuration file called", x)
            global CURRCONFIG
            CURRCONFIG = True
            return x
...
def ask_exp00(exp):         
   ...
   #loop to show all the directories inside this configuration
   listWithAllDir = []
...
def get_purpose():
    ...
    try:
        ...
        os.mkdir(dirName)
        #print(PURPLE + "Directory " , dirName ,  " Created " + NORMAL)
        ...
        #print("\n\n Purpose\n")
        #print(PURPLE +"Directory " , dirName ,  " already exists"+NORMAL)        
        f = open(dirName + "/.recall_purpose.txt", "r")
...
def get_currepo():
    ...
    print("\n --> Are we in a git/svn working tree? And from which remote repository was NEMOGCM cloned? Try to get it from git/svn. Accept with [ENTER] or modify accordingly:\n")
...
def get_nemorepo():
    print("\n\n\n --> What is the URL of the repository you installed NEMO from? Try to get it from git/svn. Accept with [ENTER] or modify accordingly:")    
    if(LNGIT==True):
        getIndexHash=""
...
def get_nemorevision():
    ...
    print("\n\n\n --> What is the NEMO revision you're currently using? Try to get it from git/svn history. Accept with [ENTER] or modify accordingly:")
    if(LNGIT == True):
        try:
            proc = subprocess.check_output(["git", "log", "--pretty=oneline", "--abbrev-commit", "--reverse"])
            proc = str(proc)
...
def get_branch():
    print("\n")
    print("--> Which branch are you using? Try to get it from git/svn. Accept with [ENTER] or modify accordingly")
    if (LNGIT==True):
        repo = git.Repo(".", search_parent_directories=True)
...
def get_components():
    ...
    print("\n --> What NEMO components are we using? Try to get a comma-separated list from ../cfg.txt. Accept with [ENTER] or modify accordingly: \n")
    #get file path for /cfg.txt
    #currentDirectory = os.getcwd()
    currentDirectory = getNemoConfig() #returns ~/NEMOGCM/CONFIG
    confName = os.path.basename(currentDirectory)
    
    #cfgPath = open(currentDirectory + "/cfg.txt", "r")
    #print("Nemo config is ",getNemoConfig())
...
def get_resolution():
    ...
    print("\n --> What is the resolution of the horizontal grid? \n\n")
...
def get_hgridtype():
    ...
    print("\n --> Horizontal grid type, e.g. ORCA for a global tri-polar grid. Accept our guess with [ENTER] or modify accordingly:\n\n")
...
def get_vgridtype():
    ...
    print("\n --> Vertical grid type. Accept our guess with [ENTER] or modify accordingly:\n\n")
...
def get_hgridpt():
    ...
    allCFG = allCFG.replace(" ", "")
#
    print("\n --> How many grid points do we have in the horizontal? Accept our guess with [ENTER] or modify accordingly:]\n\n")
...
def get_vgridpt():
    ...
    print("\n --> How many grid points do we have in the vertical? Accept our guess with [ENTER] or modify accordingly:\n\n")
...
def get_atmos():
    ...
    print("\n --> How is the surface boundary condition defined? Accept our guess with [ENTER] or modify accordingly:\n\n")
...
def get_oceanrdt() :
    ...
    print("\n --> How long is the general ocean time step for dynamics? Accept our guess with [ENTER] or modify accordingly:\n\n")
...
def check_confdir():     
#    # Check if we are in a configuration sub-folder in NEMOGCM/CONFIG:
    pathLst=[]
    currentDirectory = os.getcwd()
...
def check_prereq():
    if(EXPref != "EXP00"):
        errmsg("No EXP00 folder with default settings was found. Please create one and re-run this script.")
    else:
        cwflag = 0
        check_confdir()
...
Note: *origin/HEAD* in the output listing is not a branch in its own but points to the default branch (master branch in most cases).
    ...
    if (LNGIT==True) and (CURRCONFIG==False):
        READMEOUT = str(pathS) + "/CONFIG/" + exp + "/" + EXPref + "/README_"+exp+".md"
    elif (LNSVN==True) and (CURRCONFIG==False):
        READMEOUT = str(pathS) + "/NEMOGCM/CONFIG/" + exp + "/" + EXPref + "/README_"+exp+".md"
