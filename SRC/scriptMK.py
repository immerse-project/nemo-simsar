...
def mainconfig():
     ...
     check_inputIni_exists()
     #check if input.def is part of this directory
     if check_inputIni_exists() == True:
...
def get_components():
    ...
    if release3 == True:
        ...
        cfgPath = open(currentDirectory + "/ref_cfgs.txt", "r")        
        #cfgPath = open("../../ref_cfgs.txt", "r")        
        #cfgPath2 = open(currentDirectory + "/work_cfgs.txt", "r")
    #:MMS}
...
#MMS:{    
     ...
     for x in glob.glob('cpp_'+exp+'.fcm', recursive=True):
        matchPath = x
